...
selected_ticker = st.sidebar.selectbox("Select REIT", ranking_df["ticker"].tolist())
...
tab_rank, tab_score, tab_financials, tab_timeseries = st.tabs(
    ["Ranking", "Score", "Financial Statements", "Time Series (REITs)"]
)
...
with tab_rank:
    ...
    ranking_view = ranking_view.sort_values("final_distress", ascending=False).reset_index(drop=True)
